...
function createrow(){
    var con = $('con');
    var row = createddiv('row'); //创建div className=row
...
function fail(){
    clearInterval(clock);
    confirm('你的最终得分为 ' + parseInt($('right').innerHTML) );
}
...
function judge(events){
    if(events.target.className.indexOf('black') != -1){
        events.target.className = 'cell';
